...
export default function CommunityPage() {
  ...
  async function handleFollow(targetId) {
    ...
    try {
      if (userProfile?.following?.includes(targetId)) { await unfollowUser(currentUser.uid, targetId); } else { await followUser(currentUser.uid, targetId); }
      await fetchUserProfile(currentUser.uid); loadUsers();
    } catch { toast.error("Failed"); }
  }

  const filtered = users.filter((u) => (u.displayName||u.username||u.email||"").toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  return (
    <div className="page community-page">
      ...
      {filtered.length === 0 ? <div className="empty-state"><p>{t("noUsersFound")}</p></div> :
        <div className="community-grid">{filtered.map((user) => {
          const isFollowing = userProfile?.following?.includes(user.id);
          return (
            <div key={user.id} className="community-card">
              <div className="community-avatar">{user.avatarURL ? <img src={user.avatarURL} alt="" /> : <span>{(user.displayName||user.email||"U")[0].toUpperCase()}</span>}</div>
              ...
              {currentUser && <button onClick={() => handleFollow(user.id)} className={isFollowing ? "btn-secondary btn-sm" : "btn-primary btn-sm"}>{isFollowing ? t("unfollow") : t("follow")}</button>}
            </div>
          );
        })}</div>
      }
